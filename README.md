...
This command line tool utilizes the [FortiSession library](/fortisession) to parse the text output of `diagnose sys session list` command.
...
The session fields can be displayed in different formats controlled by "format string" given as command line parameter (`-o` or `--output`). To learn how to specify the output, see [Output format description](/fortisession/fortiformatter).

All sessions can be displayed or only interesting sessions can be selected using the "filter string" given as another command line parameters (`-f` or `--filter`). To learn how to use the conditions to select the right sessions, see [Condition format description](/fortisession/forticonditioner).
...
This feature has been simplified and moved to a [dedicated plugin](/plugins/merge/#another-example-find-the-sessions-that-were-removed).
